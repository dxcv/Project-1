...
col_query = "select name from syscolumns where id = object_id('{}')".format('Stock_MoneyFlowsIndustry_Daily')
cols = dbqu.ExecQuery(col_query)

df = DataAPI.MktIndustryFlowGet(tradeDate=date)
df.columns = [x[0] for x in cols]
df.to_sql('Stock_MoneyFlowsIndustry_Daily',dbqu.conn,if_exists='append',index=False,chunksize=1000)