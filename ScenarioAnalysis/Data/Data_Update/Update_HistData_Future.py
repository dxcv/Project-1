...
# 清楚当日已有数据
query0 = "delete from [HistData_Future_Continuing] where tradingDate='{}'".format(date)
...
for m in range(0, len(productID)):
    query2 = "select top 2 * from FUT_EOD_CONTINUING where productID='{0}' and tradingDate<='{1}' order by tradingDate desc".format(productID[m][0],date)
